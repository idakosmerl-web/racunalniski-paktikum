...
\column{0.45\textwidth}
				...
				\pause
				 ...
				 \coordinate (T') at ({-\tt},{\td});
				 \draw(T') arc[start angle=330, end angle=510, radius=2] node[right] {\scriptsize $m$};
				 \pause
				 % Naloga 3.4.3.: Definirajte točko T'' in narišite lok n z oznako.
				 \coordinate (T'') at ({\tt},{\td});
				 \draw(T'') arc[start angle=210, end angle=390, radius=2] node[left] {\scriptsize $n$};
				 \pause
				 % Naloga 3.4.4.: Definirajte in narišite točko C.
				 \coordinate [label={[blue, above right]:$C$}] (C) at (0,{\t});
				 \draw (C) circle (1.5pt);
				 % Naloga 3.4.5.: Narišite premico skozi točki T in C.
				\draw (T) -- (C) node[midway, above] {$p$};
\end{tikzpicture}
\end{columns}
\end{frame}
