...
\begin{columns}
	...
	% Spodnje je za nalogo 3.4.
			...
			\draw[blue] (T) circle (2pt);
			 ...
			 \coordinate<3-> (A') at ({-\tt},{\td});
			\draw<3->[gray, thin] (A') arc[start angle=300, end angle=420, radius=2] node[left] {\scriptsize $k$};

			\coordinate<4-> (T') at ({-\tt},{\td});
			\draw<4-> [gray, thin] (T') arc[start angle=300, end angle=420, radius=2] node[left] {\scriptsize $m$};

			\coordinate<5-> (T'') at ({-\tt},{\td});
			\draw<5-> [gray, thin] (T'') arc[start angle=120, end angle=240, radius=2] node[right] {\scriptsize $n$};

			 % Točka A
			\coordinate<6-> [label=below left:{\scriptsize $A$}] (A) at ({-\t},0);
			\draw<6-> (A) circle (1.5pt);
			\coordinate<6-> [label=below right:{\scriptsize $B$}] (B) at ({\t},0);
			\draw<6-> (B) circle (1.5pt);
			 ...
			 \coordinate<7-> (C) at (0,0);
			\draw<7-> [blue] (C) circle (2pt);

			\draw<8-> [red, very thick] (T) -- (C);
